...
> Spec fonctionnelle du module **formules** ([Q43](./QUESTIONS_OUVERTES.md)). Statut : **volets 1 (coefficients) ET 2 (indicateurs/KPI) IMPLÉMENTÉS** (2026-06-24). Inspiration ergonomique : l'éditeur de formules de Pigment.
>
> **Implémentation volet 1** : moteur pur `prototype/rust/src/formula.rs` (lexer/parser/AST → compilation `(SQL, CoeffJoins)` + interpréteur f64 pour la preview) ; bibliothèque `prototype/rust/src/coefficients.rs` (table `dim_coefficient`, seed des natifs comme formules, résolution, API REST) ; intégration `rules.rs` (enum `Coefficient` → `Constant` | `Named`, résolution déléguée). Front : `web/src/pages/CoefficientsPage.tsx` (éditeur + preview) + menu Coefficient de `RulesPage` alimenté par la bibliothèque.
>
> **Implémentation volet 2** : `prototype/rust/src/indicators.rs` — postes (`dim_aggregate`) + indicateurs (`dim_indicator`) ; compilation d'un indicateur en **une** requête au grain (`SUM(amount) FILTER (WHERE …)` par poste, LEFT JOINs partagés pour les traversées `via`/`ref`/`attr`, défaut 0, jamais réinjecté dans `fact_entry`) ; `IndicatorResolver` (réutilise le moteur de `formula.rs`, gère les renvois entre indicateurs avec détection de cycle) ; API REST `/api/aggregates`, `/api/indicators` (+ `/operands`, `/preview`). Front : `web/src/pages/IndicatorsPage.tsx` (sous-onglets Postes + Indicateurs, éditeur de sélection, formule + grain + preview live au grain). Tests : **108 unitaires lib** + `pipeline` (16) / `rules` (15) verts.
>
