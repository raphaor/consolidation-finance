...
| Q24 | **Éditeur de règles de consolidation** : modèle de composition des écritures automatiques (déclencheur, conditions, contrepassation, imputation). Démarre par éliminations interco + participations. | EDB §3.4 | POST | EN RÉFLEXION | **Spec rédigée** dans [`REGLES_CONSO.md`](./REGLES_CONSO.md) (2026-06-18). Modèle : règle = scope périmètre + N opérations ; opération = sélection de grains → facteur (coefficient × multiplicateur) → destination (hériter / surcharger / vider par dimension) ; niveau d'injection = niveau de sélection (après automatismes, sans re-déclencher). Exemple interco détaillé (4 opérations, scope croisé entity+partner). Questions R1–R7 ouvertes dans le doc. |
