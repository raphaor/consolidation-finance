...
| Q26 | **Élaboration de la consolidation** : mécanismes détaillés au fil de l'eau. | EDB §3.4 | TRANCHÉE (partiel) | Pipeline en **4 niveaux** (corporate → reclassifié → converti → consolidé). **Reclassifications de périmètre AVANT conversion** (décision 2026-06-15, simulation dans [`ANALYSE_RECLASSIFICATION_CONVERSION.md`](./ANALYSE_RECLASSIFICATION_CONVERSION.md)). Raisons : (1) traçabilité — évite les écarts F80/F81 orphelins quand F00 est reclassifié ; (2) sortie de périmètre propre en devise fonctionnelle (collapse F00+F20→F98 avant conversion, pas d'absorption d'écarts) ; (3) implémentation simple (relabeling de flux en fonctionnel, puis conversion uniforme). Résultat numérique identique entre les deux approches, mais la structure est nettement plus saine en avant. **Capturé** : modèle des flux + écarts F80/F81 ; variations de périmètre F01/F98 ; fusion F07/F70 ; application des méthodes (globale, proportionnelle). **Reste ouvert** : niveau d'application des règles (interco etc.) ; mise en équivalence détaillée (sortie du MVP — voir Q-E dans la simulation). |
